...
public class SalePanel extends JPanel
		implements ActionListener, ListSelectionListener, KeyListener, MouseListener, FocusListener {
	...
	private JLabel lblTotalValue = new JLabel("P0.00");
	...
	public SalePanel() {
		...
		add(lblTotalValue, "cell 4 4,alignx right,aligny center");
	...
	private boolean allValidInputs() {
		...
		if (StringUtils.isEmptyOrWhitespaceOnly(txtQuantity.getText())) {
			...
			int prodQtyLeft = Integer
					.parseInt(String.valueOf(tblItemSelection.getValueAt(tblItemSelection.getSelectedRow(), 2)));
			...
			try {
				if (Integer.parseInt(txtQuantity.getText()) <= 0) {
					...
					txtQuantity.setBackground(Color.YELLOW);
					valid = false;
				} else if (Integer.parseInt(txtQuantity.getText()) > Integer
						.parseInt(String.valueOf(tblItemSelection.getValueAt(tblItemSelection.getSelectedRow(), 2)))) {
					lblItemSelectionError.setText("Insufficient stock.");
					txtQuantity.setBackground(Color.YELLOW);
					valid = false;
				} else if (prodNameList.indexOf(prodName) != -1) {
					if (prodQtyList.get(prodNameList.indexOf(prodName))
							+ Integer.parseInt(txtQuantity.getText()) > prodQtyLeft) {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(btnAddToCart)) {
			if (allValidInputs()) {
				if (prodNameList.contains(
						String.valueOf(tblItemSelection.getModel().getValueAt(tblItemSelection.getSelectedRow(), 0)))) {
					int index;

					index = prodNameList.indexOf(String
							.valueOf(tblItemSelection.getModel().getValueAt(tblItemSelection.getSelectedRow(), 0)));

					sellingprice = pm.getSellingPrice(String
							.valueOf(tblItemSelection.getModel().getValueAt(tblItemSelection.getSelectedRow(), 0)));
		...
		if (e.getSource().equals(btnRemove)) {
			if (tblCart.getSelectedRow() != -1) {
				...
				totalQty -= Integer
						.parseInt(String.valueOf(tblCart.getModel().getValueAt(tblCart.getSelectedRow(), 1)));
		...
		lblTotalValue.setText("P" + String.format("%.2f", sum));
	}
